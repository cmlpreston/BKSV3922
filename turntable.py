...
class movement(Enum):
    ...
    TPR = 14    # Target position reached
...
class turntable:                                # Class for B&K Type 3922 turntable equipped with Trinamic Stepper Motor with controller
    ...
    def gen_command(self, command, degrees=0, position=0, vel_tup=(None, None)):
        ...
        switcher = {
            ...
            movement.TPR: self.add_checksum( pack('>BBBBi', self.motor_address, 138, 0, 0, 0))  # Request target position reached
        }
...
class turntable_command:
    ...
    def nudge_left(self,deg=1):
        ...
        t.start()
        t.join()
        #t = trinamic_control(self.turntable.ser, self.turntable.gen_command(movement.TPR), self.ser_lock)
        #t.start()
        #t.join()

        #if not __debug__:
        #    while not ((t.response_value[3] == 138) and (t.response_value[2] == 128))
        #        # read second response, sent when we have reached the target position, no payload
        #        t = trinamic_control(self.turntable.ser, None, self.ser_lock).read_response()
        #        t.start()
        #        t.join()
        ...
        if command in ('M', 'K', '[', ']','1','2','3','R', 'r', 'L', 'l', 'S', 's', 'O', 'o', 'Z', 'z', 'W', 'w', 'E', 'e'):
            ...
            if command in ('G', 'g'):
                ...
                try:
                    ...
                    continue
                print("position is {}\n".format(position))
                turntable_command(t,ser_lock).set_pos(position)
